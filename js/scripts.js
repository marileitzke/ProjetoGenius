...
_gui.switch.addEventListener("click", () => {
	...
	//Variáveis que quando o jogo começa sempre devem estar com valor padrão//
	...
	_gui.led.classList.remove("gui__led--active"); //Garante que quando o usuário desligue o jogo, strict também seja desligado
});

_gui.strict.addEventListener("click", () => {
	if (!_data.gameOn) return; //Caso gameOn seja false, vai retornar sem executar a ação abaixo;
...
const setScore = () => {
	...
	const display = "00".substring(0, 2 - score.length) + score; //Garante que sempre vai aparecer 2 dígitos no display
	_gui.counter.innerHTML = display; //Essa função sempre vai mostrar o score
};
...
const newColor = () => {
	...
	_data.gameSequence.push(Math.floor(Math.random() * 4)); //Push coloca o valor gerado no final do array, Math.floor arredonda o valor pra baixo, Math.random gera um número aleatório
	_data.score++;
